...
app_mode = st.sidebar.selectbox('Select Page',['Home','Loan Prediction'])
if app_mode=='Home':
    st.title('LOAN PREDICTION :')
    st.image('loan_image.jpg') 
    st.write('@DSU - Assignment (Streamlit) - LOAN Application PURPOSES')  
    ...
    st.subheader('Sir/Madam, Please fill ALL neccesary informations in order to determing your Loan Application Status!')
    ...
    get_value(Gender, gender_dict), get_fvalue(Married), data1['Dependents'][0], data1['Dependents'][1], data1['Dependents'][2], data1['Dependents'][3], get_value(Education,edu), get_fvalue(Self_Employed), data1['Property_Area'][0], data1['Property_Area'][1], data1['Property_Area'][2]]
    ...
    if st.button("Predict"):
        ...
        if prediction[0] == 0 :
            st.error(
    'According to our Calculations, you DO NOT qualify or the Bank Loan - Sorry!!!'
    )
            st.markdown(
